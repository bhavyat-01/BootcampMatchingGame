...
*/
function initGame() {
    //click listener for resetButton
...
function gameStart(){
    ...
    //stores the resetButton and removes it from html
    let resetButton = document.getElementById('restart-btn') 
    resetButton.remove();

    /*
        - Create two cards of each symbol, (go in order and create Card twice)
    */
    ...
    //shuffles cards because all symbols will be consecutive
    shuffleArray(cards);

    //adds cardDivs to the game-board div one by one
    for(let i=0; i<16; i++){
        document.getElementById('game-board').appendChild(cards[i]); 
    }
...
function createCard(symbol) {
    ...
    //create div for card, add className, 
...
function flipCard(card) {
    ...
    symbolText.textContent = card.dataset.symbol;
    card.appendChild(symbolText);
    ...
    if(firstCard == null){
        ...
        secondCard = card; 
        //lock board so users cannot click on other cards
        lockBoard = true; 
        checkForMatch(); 
    }
...
function checkForMatch() {
    ...
    if(firstCard.dataset.symbol === secondCard.dataset.symbol){
        ...
        countCardsMatched = countCardsMatched+1; 
    }
    else{
        unflipCards(); 
    }

    console.log(countCardsMatched); 
    if(countCardsMatched === 8){
    
        //delete all cards 
        let allCards = document.getElementById('game-board').querySelectorAll('div'); 
